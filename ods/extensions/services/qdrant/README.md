...
| `QDRANT_API_KEY` | *(empty)* | Optional API key for authenticating REST and gRPC endpoints (auto-generated by setup.sh) |

When `QDRANT_API_KEY` is configured, client requests must supply the `api-key: <token>` HTTP header. Unauthenticated probes to `/readyz` and `/healthz` remain accessible for Docker and orchestrator healthchecks.

## API Endpoints

| Endpoint | Method | Description |
|----------|--------|-------------|
| `GET /` | GET | Root service information |
| `GET /readyz` | GET | Unauthenticated readiness healthcheck |
