...
MAX_STEPS = 12
CSP = ("default-src 'self' data: blob:; connect-src 'self'; img-src 'self' data: blob:; "
...
def strict_json(raw):
    def pairs(values):
        ...
        for key, value in values:
            ...
            result[key] = value
        return result
    return json.loads(raw, object_pairs_hook=pairs, parse_constant=lambda _: (_ for _ in ()).throw(Invalid('nonfinite')))
...
def printable(value, chars, size):
    return isinstance(value, str) and 1 <= len(value) <= chars and not any(unicodedata.category(c).startswith('C') or unicodedata.category(c) in ('Zl','Zp') for c in value) and len(value.encode('utf-8')) <= size

def validate_request(value):
    exact(value, ('schemaVersion','action','siteId','sha256','viewport','steps'))
    if type(value['schemaVersion']) is not int or value['schemaVersion'] != 1 or value['action'] != 'inspect':
        raise Invalid('invalid request')
    digest = value['sha256']
    if not isinstance(digest, str) or not re.fullmatch('[a-f0-9]{64}', digest) or value['siteId'] != 'site-' + digest[:24]:
        raise Invalid('invalid identity')
    exact(value['viewport'], ('width','height'))
    if any(type(n) is not int or not 240 <= n <= 1920 for n in value['viewport'].values()):
    ...
    for step in steps:
        ...
        if set(locator) == {'selector'}:
            ...
            if not printable(selector, 256, 1024) or '>>' in selector or re.match(r'^[A-Za-z_-]+=', selector):
                raise Invalid('invalid selector')
        else:
            exact(locator, ('role','name','exact'))
            if locator['role'] not in ('button','link','checkbox','radio','textbox','combobox','heading','tab','switch') or locator['exact'] is not True or not printable(locator['name'], 120, 480):
...
def validate_bundle(bundle):
    ...
    for entry in files:
        ...
        name = entry['path']
        if not isinstance(name, str) or len(name) > 512 or len(name.split('/')) > 12 or name <= previous or any(not re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9._-]{0,127}', p) or p in ('.','..') for p in name.split('/')):
            raise Invalid('invalid file path')
        if not isinstance(entry['base64'], str) or len(entry['base64']) > (MAX_FILE + 2) // 3 * 4:
        ...
        path = name.encode()
        digest.update(len(path).to_bytes(4,'big')); digest.update(path)
...
def failure(code, request=None):
    return {'schemaVersion':1,'kind':KIND,'status':'failed','errorCode':code,
            **({'siteId':request['siteId'],'sha256':request['sha256'],'planSha256':plan_hash(request)} if request else {}),
            'scope':SCOPE}
