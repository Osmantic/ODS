...
// after the model's own `own` edits); they are reported, never held again.
// Returns {note, hold} where hold lists the model's matched edits to keep.
export function editRecovery(content, edits, {held} = {}) {
  ...
  const own = held ? hunks.slice(0, held.own) : hunks;
  ...
  if (held) {
    sentences.push(`The ${plural(held.indices.length, 'held edit')} from the earlier call ${held.indices.length === 1 ? 'was' : 'were'} included and not applied; ${held.indices.length === 1 ? 'it is' : 'they are'} no longer held, so include ${held.indices.length === 1 ? 'it' : 'them'} again with your corrected edit. Do not rewrite the whole file.`);
...
// after the model's own. A held edit is dropped when the new call re-sends
// its oldText, or when it no longer matches alone and uniquely in the merged
// call (for example because a new edit overlaps its region). The new edit
// always wins. Returns undefined when nothing held can be added.
export function mergeHeldHunks(content, ownEdits, held) {
  ...
  let kept = held.filter(hunk => !resent.has(normalizeToLF(hunk.oldText)));
  ...
  return {edits: [...ownEdits.map(edit => ({oldText: edit.oldText, newText: edit.newText})),
    ...kept.map(hunk => ({oldText: hunk.oldText, newText: hunk.newText}))], held: kept.map(hunk => hunk.index)};
}

export function mergedEditNote(ownCount, heldIndices) {
  const total = ownCount + heldIndices.length;
  return `${EDIT_NOTE_PREFIX} Applied ${plural(total, 'replacement')} in one edit: yours plus ${heldIndices.length} held from the rejected call (its ${indexRanges(heldIndices)}).`;
}
