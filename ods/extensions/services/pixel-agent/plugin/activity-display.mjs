...
export const ACTIVITY_CONTRACT = 'Portal automatically displays progress from actual tool calls. Start the task with the tools it needs; a separate progress call is not required. Use pixel_ods_activity optionally for a useful verified finding or blocker that the tool activity does not explain, in the owner language, with {message:"..."}. Updates do not execute the task or prove completion. Do not repeat tool activity, expose private reasoning, secrets or raw tool output, or claim success before verification.';
