...
for(const mode of ['plain','wrapped','sandbox','deferred']) test(`real SDK completed exec guidance: ${mode}`,{skip:!pkg||(mode==='sandbox'&&!image),timeout:90000},async()=>{
  ...
  writeFileSync(join(plugin,'openclaw.plugin.json'),JSON.stringify({id:'exec-fixture',activation:{onStartup:true},configSchema:{type:'object',properties:{}}}));
  writeFileSync(join(plugin,'index.mjs'),`
    ...
    export default {id:'exec-fixture',register(api){
      ...
      api.on('tool_result_persist',(event,ctx)=>{const result=guard.toolResultPersist(event,ctx);save({hook:'persist',guidance:String(JSON.stringify(result?.message?.content)).includes('[ODS Pixel execution]')});return result;});
    }};
  `);
  ...
  try{
    ...
    for(let i=0;i<2;i++)assert.match(JSON.stringify(toolResults[i].content),new RegExp('Exec returned completed with exit code '+i),JSON.stringify({mode,hooks,content:toolResults[i].content}));
    assert.match(JSON.stringify(toolResults[1].content),/sdk-failed/,'nonzero process evidence remains visible');
    assert.match(JSON.stringify(toolResults[2].content),/\[ODS Pixel Python syntax\]/,'specific repair guidance reaches the actual next provider request');
