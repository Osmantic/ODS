...
import {createToolLoopGuard, UNPUBLISHABLE_PREVIEW_DIRECTORY_STEP} from '../plugin/tool-loop-guard.mjs';
...
// check always refuses (the first failure). After mv, publishing before a
// read of the moved entry was refused too (the second failure).
...
test('mac-mini round 106: a hidden site directory is never prescribed for publication',()=>{
  ...
  assert.doesNotMatch(written.text,/tool_call/);
...
test('no next step prescribes a directory that the publication check refuses',()=>{
  for(const [file,shown] of [['.site/index.html',false],['site/.draft/index.html',false],['my site/index.html',false],['site-2/index.html',true]]) {
    const {invoke}=fixture();
