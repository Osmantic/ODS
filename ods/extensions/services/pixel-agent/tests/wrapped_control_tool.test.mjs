...
test('strixy round 107: wrapped tool_describe calls 9 and 10 get the direct route free, and the fuse still holds', async t => {
  ...
  assert.ok(inspectText(8).includes('Next step: call pixel_ods_workspace_preview_inspect (a tool in your list; call it by name) with exactly these args: ' +
    `${JSON.stringify(READY)}; these are your own identifiers and locators in the required shape.`), inspectText(8));
...
test('rejected inspection arguments for an earlier snapshot are never offered back as ready to send', async t => {
  ...
  assert.ok(bound.result.content[0].text.includes(`with exactly these args: ${JSON.stringify(ready)}; these are your own identifiers and locators in the required shape.`),
