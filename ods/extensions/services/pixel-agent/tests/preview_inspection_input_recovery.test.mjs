...
import {argumentCorrection,createWorkspacePreviewInspectTool,correctedInspectionArgs,normalizeWorkspacePreviewInspectionParams} from '../plugin/workspace-preview-inspect.mjs';
...
test('invalid inspection arguments never contact the broker or report it unavailable',async()=>{
  ...
  for(const params of [observed,unsupportedRole,wrongBinding,wrongViewport,null,{}]) {
    ...
    // the tool is directly visible, so neither control route is named (#6747).
    assert.doesNotMatch(result.content[0].text,/tool_describe|tool_call/);
    assert.match(result.content[0].text,/Next step: call pixel_ods_workspace_preview_inspect /);
...
test('a plan rejected only for the form of its locators gets itself back, ready to send, for the current publication',async()=>{
  ...
  for(const [params,corrected] of [
    ...
    // Integration with #6748: its ready-call wording (the direct route, by name).
    assert.ok(text.includes(`Next step: call pixel_ods_workspace_preview_inspect (a tool in your list; call it by name) with exactly these args: ${JSON.stringify(corrected)}; these are your own identifiers and locators in the required shape.`),text);
