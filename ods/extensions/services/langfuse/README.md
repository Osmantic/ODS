...
- **Isolated storage**: Dedicated PostgreSQL, ClickHouse, Redis, and SeaweedFS services — no shared state with other extensions
...
| `LANGFUSE_S3_ACCESS_KEY` | *(generated)* | S3 access key for event uploads (SeaweedFS) |
| `LANGFUSE_S3_SECRET_KEY` | *(generated)* | S3 secret key for event uploads (SeaweedFS) |
...
| `data/langfuse/seaweedfs/` | `/data` | Event upload bucket (`langfuse-events`) |

**Upgrading from MinIO:** older installs stored the bucket in `data/langfuse/minio/`. SeaweedFS does not read that format, so the new store starts empty. Traces in ClickHouse and PostgreSQL are unaffected. Delete the old directory by hand when you no longer need it.
...
| `ods-langfuse-seaweedfs` | `chrislusf/seaweedfs` | Object store for event uploads |
...
```bash
docker compose ps langfuse langfuse-worker langfuse-postgres langfuse-clickhouse langfuse-redis langfuse-seaweedfs
