...
def test_signed_header_body_unchanged_response_metrics_all_paths(router, kind):
    ...
    write(); probe = str(uuid.uuid4()); arm(client, probe)
    ...
    body = json.dumps(payload).encode(); header = signed(probe, body); requests = []
    ...
    asyncio.run(mod.app.state.http.aclose()); mod.app.state.http = httpx.AsyncClient(transport=httpx.MockTransport(backend))
...
def test_sse_chunk_boundaries_and_overflow_do_not_retain_raw_content():
    rows=[]; observer=ProbeStream(rows.append)
    for chunk in [b':keepalive\n\nda',b'ta: {"usage":{"prompt_tokens":1}}\r',b'\n\ndata: [DONE]\n\n']:observer.feed(chunk)
    assert rows == [{'usage':{'prompt_tokens':1}}]
    observer.feed(b'x'*(observer.MAX_PENDING+1));assert not observer.available and observer.pending == b''
