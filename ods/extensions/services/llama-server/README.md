...
- passes `LLAMA_PARALLEL` as `--parallel`, or 1 when it is unset, except for the shared-slot layout below. The launchers no longer pass `--parallel` themselves.
- uses the shared-slot layout, `--parallel 2 --kv-unified --ctx-checkpoints 64 --cache-ram 0`, when all of these hold: `LLAMA_PARALLEL`, `LLAMA_ARG_CTX_CHECKPOINTS` and `LLAMA_ARG_CACHE_RAM` are unset; the Mac has at most 16 GiB (`sysctl hw.memsize`); the binary has b9014's `--cache-idle-slots`; and the model is a hybrid (recurrent) model whose layout `config/model-library.json` has reviewed, with checkpoints small enough that 2 slots × 64 checkpoints, plus the second slot's own state, use no more memory than 32 checkpoints plus llama.cpp's 8 GiB RAM prompt cache could. That holds up to about 83 MiB per checkpoint: Qwen3.5-2B/4B/9B and Nemotron 3 Nano 4B qualify, Qwen3.5-27B (150 MiB) does not. Anything else keeps the one-slot default.

  With one slot, any other client (Talk, Hermes, Open WebUI, a second Pixel chat) takes over the slot, and the conversation it replaces survives only in the RAM prompt cache. On the 16 GB Mac mini that cache reached 8017 MiB next to 7.3 GiB of Metal buffers, with swap in use. `--kv-unified` keeps both slots in one KV buffer that is still `CTX_SIZE` long, so either slot can use the full context. b9014 saves idle slots into the RAM cache and clears them on every new task when a cache exists, so `--cache-ram 0` is what keeps the second slot resident. 64 checkpoints keep the one at the end of a shared system prompt for about 30 tool calls instead of about 15. On the Mac mini with Qwen3.5-9B at 64K context, a 27-call session followed by a new chat with the same system prompt took 334.9 s with one slot, 262.1 s with 64 checkpoints and 250.4 s with 64 checkpoints and no RAM cache. With another client's request in the middle of the session it took 361.7 s with one slot, 453.1 s with 64 checkpoints and no RAM cache on one slot (the session prefix was lost), and 274.0 s with the shared-slot layout.

  Trade-offs: the two slots share `CTX_SIZE` cells. If both conversations together outgrow it, llama.cpp clears the idle slot, and without a RAM cache that conversation is re-processed from the start. Two requests at once now run together instead of one waiting for the other, and share the Mac's memory bandwidth while both decode. Set `LLAMA_PARALLEL=1` to keep the one-slot default.
