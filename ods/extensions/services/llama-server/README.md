...
- **Full GPU residency**: ODS chooses settings that keep every layer on the GPU (a smaller `LLAMA_ARG_UBATCH`, a 512 MiB `LLAMA_ARG_FIT_TARGET` instead of llama.cpp's 1024 MiB default, a quantized KV cache, or a context no lower than the 64K agent floor), budgeting GPU memory other processes already hold, such as a desktop drawn on the NVIDIA GPU. The host agent reads the load log after every model activation and, through its residency watcher, after installer starts, reboots and restarts. A model that lands partly on the CPU is relaunched once with settings planned from llama.cpp's own numbers. If it is still partial, a dashboard activation rolls back, except when other processes hold the memory it needs, when the running model is re-activated, or on a GPU below 8 GB (the estimate is not calibrated there): those loads keep running and `runtime.placement` reports them. Memory held by other processes never refuses an activation. An operator's own `LLAMA_ARG_UBATCH` and `LLAMA_ARG_FIT_TARGET` are kept and never re-planned. `N_GPU_LAYERS` stays `auto` so llama.cpp never runs out of memory; a numeric value that limits GPU layers fails activation. MoE expert offload that a runtime profile declares (`LLAMA_ARG_N_CPU_MOE`) is the only allowed exception
