...
].map(artifact => ({ ...artifact, termsPreview: modelTermsFixture(artifact.id) }))
...
test('imports the exact filtered artifact and retains backend import guards', async () => {
  ...
  expect(JSON.parse(request[1].body)).toEqual({repoId: 'org/model', artifactId: 'q8', termsAcknowledgement: {termsDigest: 'a'.repeat(64), acknowledged: true}})
