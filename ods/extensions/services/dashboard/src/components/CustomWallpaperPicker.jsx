...
export default function CustomWallpaperPicker() {
  const {theme, wallpapers = [], addWallpaper, removeWallpaper, wallpaperError, wallpaperMotion, setWallpaperMotion, wallpaperFit = 'cover', setWallpaperFit, wallpaperFitError} = useTheme()
  ...
  return <div className="custom-wallpaper-actions">
    ...
    <label className="wallpaper-motion">Wallpaper fit <select className="rounded border border-theme-border bg-theme-bg p-2" value={wallpaperFit} onChange={event => setWallpaperFit(event.target.value)}><option value="cover">Fill screen (crop edges)</option><option value="contain">Fit entire image (allow borders)</option></select></label>
    {wallpaperFitError && <p role="alert">{wallpaperFitError}</p>}
