...
export default function PixelCommandSearch({ onInsert, onNewTask }) {
  ...
  const entries = [
    ...
    { title: 'Assistant settings', detail: 'Connections, access and sharing', icon: Settings, run: () => navigate('/pixel/settings') },
  ...
  return <dialog ref={dialog} className="pixel-command-dialog" aria-label="Search assistant conversations and actions" onClick={event => { if (event.target === event.currentTarget) close() }} onCancel={() => trigger.current?.focus?.()}>
