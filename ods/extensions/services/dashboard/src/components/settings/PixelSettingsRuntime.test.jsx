...
const apply = () => button('Apply saved assistant preferences')
...
it('explains that a fresh owner store needs Save without advertising runtime activation', async () => {
  ...
  render(<PixelSettingsRuntime {...props()} />)
  expect(await screen.findByText('Save assistant preferences once to initialize runtime controls. Saving does not apply them.')).toBeVisible()
...
it('locks parent preferences during one Apply and requires matching fresh GET before success', async () => {
  ...
  expect(button('Save assistant preferences')).toBeDisabled()
...
it('continues to save sparse preferences when runtime control is unavailable', async () => {
  ...
  fireEvent.click(button('Save assistant preferences'))
  await waitFor(() => expect(screen.getByText('Preferences saved. The assistant runtime is unchanged.')).toBeVisible())
  expect(apply()).toBeDisabled()
  expect(posts(mock)).toHaveLength(1)
