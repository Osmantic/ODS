...
const states = displayName => [
  ['idle','Idle',`Breathing, curious glances and blinks. Move the pointer or click ${displayName} to interact.`],
  ...
  ['sleeping','Sleeping',`Soft breathing, curved closed eyes and floating zzz. Typing in chat or clicking ${displayName} wakes the mascot with a stretch.`],
]
export default function PortalMascotSettings() {
  const {displayName} = usePortalIdentity()
  const mascotStates = states(displayName)
  ...
  return <section className="portal-mascot-settings" aria-labelledby="portal-mascot-title">
    <h2 id="portal-mascot-title">{displayName} mascot</h2>
    <p>Customize the little character without changing the assistant or its permissions. Saved in this browser.</p>
    <label className="portal-mascot-setting"><span><strong>Show mascot</strong><small>Hide or restore {displayName}’s character across the interface.</small></span><input type="checkbox" checked={preferences.enabled} onChange={event=>update({enabled:event.target.checked})}/></label>
    ...
    <label className="portal-mascot-setting"><span><strong>Sleep after inactivity</strong><small>Time without typing in chat or clicking {displayName}. Moving the mouse or using other pages does not reset it; running tasks never sleep.</small></span><select aria-label="Sleep after inactivity" value={preferences.sleepAfterSeconds} onChange={event=>update({sleepAfterSeconds:Number(event.target.value)})}>
    ...
    <div className="portal-mascot-demo">
      <PixelMascot key={replay} preview interactive name={displayName} state={state} className="portal-settings-character"/>
      <div><h3>Try the expressions</h3><p>{mascotStates.find(([id])=>id===state)[2]}</p><small>Demo only — not live task activity. The preview remains visible when the chat mascot is hidden.</small></div>
    </div>
    <div className="portal-mascot-states" aria-label="Preview mascot state">{mascotStates.map(([id,label])=><button key={id} type="button" aria-pressed={state===id} onClick={()=>{setState(id);setReplay(value=>value+1)}}>{label}</button>)}</div>
