  ...
  return (
    <div className={compact ? 'models-refined' : 'p-3 sm:p-6 lg:p-8'}>
      ...
      {libraryScope === 'huggingface' ? (
        <section
          ref={libraryRef}
          id={`model-source-panel-${libraryScope}`}
          role="tabpanel"
          aria-labelledby={`model-source-tab-${libraryScope}`}
      ...
      <div
        ...
        role="tabpanel"
        aria-labelledby={`model-source-tab-${libraryScope}`}
        className={compact ? 'models-library' : 'grid gap-4 xl:grid-cols-[260px_minmax(0,1fr)]'}
      >
...
function ModelSourceTabs({ value, onChange, installedCount, recommendedCount, compact = false }) {
  ...
  if (compact) return <div className="portal-model-tabs" role="tablist" aria-label="Model sources">{tabs.map(tab => <button key={tab.id} id={`model-source-tab-${tab.id}`} role="tab" aria-selected={value === tab.id} aria-controls={`model-source-panel-${tab.id}`} onClick={() => onChange(tab.id)}>{tab.label}{tab.count !== null && <small>{tab.count}</small>}</button>)}</div>
  const activeStyles = {
    emerald: {
    ...
    >
      {tabs.map(({ id, label, detail, count, icon: Icon, image, tone }) => {
        ...
        return (
          <button
            key={id}
            id={`model-source-tab-${id}`}
            type="button"
            role="tab"
            aria-selected={active}
            aria-controls={`model-source-panel-${id}`}
            onClick={() => onChange(id)}
