...
describe('Invites', () => {
  ...
  test('reports clipboard denial with a manual-copy fallback', async () => {
    const fetchMock = vi.fn(async (url, options = {}) => {
      ...
      if (url === '/api/auth/magic-link/generate' && options.method === 'POST') return response({
        token: 'plain-secret-token', url: 'http://auth.ods.local/magic-link/plain-secret-token',
        expires_at: future, target_username: 'bob', scope: 'chat', reusable: false,
        token_type: 'guest', url_mode: 'auto',
      })
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent(/Clipboard access was denied/i)
  })
