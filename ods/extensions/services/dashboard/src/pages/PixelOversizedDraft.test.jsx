...
it('retains an oversized unsent draft across reload and permits editing it back under the send limit', async () => {
  ...
  fireEvent.change(screen.getByPlaceholderText('Message Assistant...'), {target:{value:text}})
  await waitFor(() => expect(JSON.parse(localStorage.getItem(CHAT_KEY)).draft).toBe(text))
  expect(screen.getByTitle('Send')).toBeDisabled()
  first.unmount()
  render(<Pixel/> )
  await screen.findByText('Available')
  expect(screen.getByPlaceholderText('Message Assistant...')).toHaveValue(text)
  expect(JSON.parse(localStorage.getItem(CHAT_KEY)).draft).toBe(text)
  expect(screen.getByTitle('Send')).toBeDisabled()
  fireEvent.change(screen.getByPlaceholderText('Message Assistant...'), {target:{value:'Edited draft'}})
