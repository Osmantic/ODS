...
test.each(['md', 'json'])('exports the displayed conversation as %s without another network request', async (format) => {
  const fetchMock = vi.fn(async url => url === '/api/talk/status'
    ...
    : sseResponse([{ type: 'complete', text: 'Answer with Unicode �', status: 'ok' }, { type: 'done' }]))
  vi.stubGlobal('fetch', fetchMock)
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Send message' }))
  await screen.findByText('Answer with Unicode �')
  ...
  expect(text).toContain('Answer with Unicode �')
