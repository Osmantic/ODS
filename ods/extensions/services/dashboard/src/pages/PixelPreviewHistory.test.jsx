...
it('opens an earlier verified publication and returns to latest without submitting work',async()=>{
  ...
  expect(screen.getByTitle('Interactive Assistant preview')).toHaveAttribute('src',`/pixel-preview/${first.siteId}/__ods_view__.html`)
  ...
  expect(screen.getByTitle('Interactive Assistant preview')).toHaveAttribute('src',`/pixel-preview/${latest.siteId}/__ods_view__.html`)
