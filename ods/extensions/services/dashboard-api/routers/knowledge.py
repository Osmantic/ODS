...
async def get_embeddings(texts: List[str]) -> List[List[float]]:
    try:
        ...
        raise HTTPException(
            status_code=502, detail=f"Embeddings service unavailable: {e}"
        )
...
def extract_text(file_content: bytes, filename: str) -> str:
    if filename.lower().endswith(".pdf"):
        ...
        raise HTTPException(
            status_code=400,
            detail="Unsupported file format. Please upload PDF, TXT, MD, or CSV.",
        )
...
async def upload_document(
    ...
    if not text.strip():
        raise HTTPException(
            status_code=400, detail="Document contains no extractable text."
        )
    ...
    if qdrant is None:
        raise HTTPException(
            status_code=503, detail="Qdrant vector database is not connected."
        )
...
async def delete_document(
    doc_id: str, request: Request, api_key: str = Depends(verify_api_key)
):
    qdrant = await get_qdrant()
    if qdrant is None:
        raise HTTPException(
