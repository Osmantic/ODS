    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            response = await client.get(
                f"{_GITHUB_API_BASE}/releases/latest",
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            resp = await client.get(
                f"{_GITHUB_API_BASE}/releases?per_page=5",
                headers=_GITHUB_HEADERS,
            )
        releases = resp.json()
        ...
        current = await asyncio.to_thread(_read_current_version)
        return {
            "releases": [{"version": current, "date": datetime.now(timezone.utc).isoformat() + "Z", "title": f"ODS {current}", "changelog": "Release information unavailable. Check GitHub directly.", "url": _GITHUB_RELEASES_URL, "prerelease": False}],
            "checked_at": datetime.now(timezone.utc).isoformat() + "Z",
