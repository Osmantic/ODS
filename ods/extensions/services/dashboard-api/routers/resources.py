...
async def service_resources(api_key: str = Depends(verify_api_key)):
    ...
    total_cpu = sum(float(s.get("cpu_percent") or 0) for s in container_stats if isinstance(s, dict))
    total_mem = sum(float(s.get("memory_used_mb") or 0) for s in container_stats if isinstance(s, dict))
    total_disk = sum(float(d.get("data_gb") or 0) for d in disk_usage.values() if isinstance(d, dict))
