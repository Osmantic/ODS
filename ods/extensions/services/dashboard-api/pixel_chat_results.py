...
class ChatResultStore:
    ...
    def close(self):
        # Release this worker's live leases before closing. Other workers must
        # still count active leases; otherwise each process can independently
        # admit MAX_ACTIVE results and overload the shared result store.
        with self.db:
            self.db.execute("UPDATE attempts SET state='unresolved' WHERE instance=? AND state='active'", (self.instance,))
        self.db.close()
    ...
    def reserve(self, key, fingerprint):
        ...
        with self.db:
            ...
            count, active, allocation = self.db.execute(
                ...
                "COALESCE(SUM(CASE WHEN state='active' THEN ? ELSE size END),0) FROM attempts",
                (MAX_RESULT_BYTES,)
            ).fetchone()
