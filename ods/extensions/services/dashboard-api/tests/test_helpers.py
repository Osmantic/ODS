...
class TestDictKeyCountSafe:
    def test_valid_counting(self):
        from helpers import dict_key_count_safe
        assert dict_key_count_safe({"a": 1, "b": 2}) == 2

    def test_invalid_inputs(self):
        from helpers import dict_key_count_safe
        assert dict_key_count_safe(None) == 0
        assert dict_key_count_safe([1, 2]) == 0
