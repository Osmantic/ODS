...
class TestDictMapKeysTransformSafe:
    def test_valid_mapping(self):
        from helpers import dict_map_keys_transform_safe
        d = {"usr": "alice", "pwd_hash": "xxx"}
        assert dict_map_keys_transform_safe(d, {"usr": "username"}) == {"username": "alice", "pwd_hash": "xxx"}

    def test_invalid_inputs(self):
        from helpers import dict_map_keys_transform_safe
        assert dict_map_keys_transform_safe(None, {"usr": "username"}) == {}
        assert dict_map_keys_transform_safe({"a": 1}, None) == {"a": 1}
