"""The PocketBase recipe must survive the real install and re-enable boundaries."""
...
def test_pocketbase_install_and_reenable_use_an_image_only_compose(test_client, monkeypatch, tmp_path):
    ...
    def accept_install(service_id):
        ...
        assert (installed / "pb_hooks/bootstrap.pb.js").read_bytes() == (library / "pocketbase/pb_hooks/bootstrap.pb.js").read_bytes()
        assert (installed / ".dockerignore").is_file()
        return True

    agent = Mock(side_effect=accept_install)
    monkeypatch.setattr(extensions, "_call_agent_install", agent)
    response = test_client.post("/api/extensions/pocketbase/install", headers=test_client.auth_headers)
    assert response.status_code == 200, response.text
    assert response.json()["restart_required"] is False
    agent.assert_called_once_with("pocketbase")
    ...
    for action in ("disable?include_data_info=false", "enable"):
        response = test_client.post(f"/api/extensions/pocketbase/{action}", headers=test_client.auth_headers)
        assert response.status_code == 200, response.text
        assert response.json()["restart_required"] is False
