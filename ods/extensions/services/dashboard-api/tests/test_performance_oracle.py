...
def test_qwen35_2b_is_the_4gb_recommendation_despite_fleet_failures(
    ...
    # decides. The residency plan is still shown: the 2B stays on the GPU at
    # 64K with ubatch 256 and a 512 MiB llama.cpp margin (native Linux).
    assert model["gpuResidency"]["decisive"] is False
    assert model["gpuResidency"]["fits"] is True
    assert model["gpuResidency"]["adjustments"] == ["ubatch 256", "fit target 512 MiB"]
