...
**Pixel** is the preferred autonomous agent on qualified Linux hosts and is included in ODS under its ODS-only source-available license. It appears directly in Open WebUI and in the Dashboard toolbar, so a normal conversation can become a tool-using agent turn without opening a separate product. **Hermes Agent** remains the portable default and rollback everywhere Pixel is unavailable or disabled. With an agent, you can give the AI a goal — "research this topic, find the three best sources, write me a summary, and save it to a file" — and it figures out the steps, uses its permitted tools, and returns the result. Hermes is reached through its auth proxy. OpenClaw is still available for older workflows, but it is deprecated and no longer a default path. See [PIXEL.md](PIXEL.md) for the host, license, security, and rollback boundaries.
