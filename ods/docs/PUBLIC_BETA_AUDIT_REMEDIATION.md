...
| PB-011: mutable workflow inputs | Pinned the 11 reported Action refs and 12 CI image indices, with digest records and regression checks. | Lock remaining workflow Python tooling and inspect shipped nested workflow templates; these changes do not make every supply-chain input immutable. |
| PB-012: whitespace/lint | Removed 162 whitespace findings from 11 Python files with equivalent ASTs; preserved required whitespace in two digest-bound format patches. Enabled React's JSX usage analysis, removed obsolete suppressions and genuinely unused bindings/imports. Dashboard lint now reports zero errors/warnings and enforces `--max-warnings 0`; all 1,596 tests and the production build pass. | Required CI and the full whitespace gate must pass again at the final candidate. No warning baseline or blanket suppression was introduced. |
| PB-013: recipe/model provenance | Corrected AudioCraft's noncommercial-model description in its README, manifest and generated catalog. Added a [57-entry model evidence inventory](MODEL_TERMS_AUDIT.md) covering 147 observed public revisions and explicit upstream conflicts/gaps. Coverage and document hashes were checked, with an independent review of the highlighted findings. | The inventory does not approve licenses or change downloads. Backfill the 34 legacy recipe records and the model catalog's missing source/terms records, validate coverage, and present required terms before downloads. |
...
| PB-018: downloaded execution | Added a fail-closed SHA-256 downloader and reviewed immutable Docker/NodeSource scripts. Linux/macOS OpenCode use reviewed artifact pins; Windows verifies the pinned ZIP before parsing/extraction. Claude/Codex require `ODS_INSTALL_AI_CLIS=true` and use exact versions, a complete npm integrity lock, disabled lifecycle scripts, and an installation receipt. Substitution/opt-in tests and isolated Linux installation/version checks passed. | Complete native macOS/Windows CLI qualification and the download inventory. OS package repositories, unpinned host-agent Python packages, container/model downloads, existing user-installed tools and other supplier inputs still require their own acceptance evidence. See [optional CLI installation](../installers/ai-clis/README.md). |
