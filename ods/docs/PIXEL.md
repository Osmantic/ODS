...
`web_fetch`, `pixel_ods_web_extract` and `pixel_ods_search_read` return
transformed, safety-marked page evidence, not the origin server's exact response
bytes. Pixel must not save that
