...
ods_pixel_install_default_agent() {
    ...
    [[ -f "$plugin_root/plugin/openclaw.plugin.json" \
        ...
        && -f "$plugin_root/host/openclaw-precheck-route.json" \
    ...
    if ! ods_pixel_run_as_owner "$owner" "$home" python3 \
        ...
        --known tool-recovery completion-recovery image-envelope compaction-export \
            ...
            precheck-route compaction-budget \
        >>"$pixel_log" 2>&1; then
    ...
    # Recover an owner-turn context overflow by trimming old tool output when
    # that alone fits the budget. The pinned route test prices tool-result text
    # at the prose rate and summarizes the history instead (21-114 s per event
    # on recorded fleet journeys).
    if ! ods_pixel_run_as_owner "$owner" "$home" python3 \
        "$plugin_root/host/openclaw_tool_recovery.py" \
        --openclaw-bin "$openclaw_bin" --precheck-route \
        --state-dir "$home/.openclaw/ods-runtime-patches/precheck-route" \
        >>"$pixel_log" 2>&1; then
        ai_bad "Pixel's context overflow route repair could not verify its package bytes. See $pixel_log."
        return 1
    fi
