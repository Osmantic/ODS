...
# Strip optional GiB/GB/G suffixes and whitespace from memory inputs.
_clean_memory_gb() {
    local val="${1:-0}"
...
ods_effective_container_memory_gb() {
    ...
    host_gb="$(_clean_memory_gb "${1:-0}")"
    docker_gb="$(_clean_memory_gb "${2:-0}")"
...
ods_default_nvidia_llama_memory_limit() {
    local memory_gb reserve_gb usable_gb
    memory_gb="$(_clean_memory_gb "${1:-0}")"
