...
if [[ "$GPU_BACKEND" == "amd" ]] && $DRY_RUN; then
    ...
    if [[ "$total_ram_mb" -gt 0 ]]; then
        ...
        # Stage the file in a private, unpredictable directory. The former
        # fixed /tmp path is writable by every local user: they could
        # pre-create it as a symlink (so `cat >` truncates a file this user
        # owns) or swap it between the write and the `sudo cp` below, which
        # would install attacker-chosen amdgpu/ttm module options into
        # /etc/modprobe.d for the next boot.
        gtt_stage_dir="$(mktemp -d "${TMPDIR:-/tmp}/ods-gtt-tuning.XXXXXX" 2>/dev/null)" || gtt_stage_dir=""
        if [[ -z "$gtt_stage_dir" ]]; then
            ai_warn "Could not create a private staging directory — skipping GTT memory tuning"
        else
            chmod 700 "$gtt_stage_dir"
            gtt_stage_file="$gtt_stage_dir/amdgpu_llm_optimized.conf"

            cat > "$gtt_stage_file" << GTT_EOF
...
GTT_EOF
            if sudo -n cp "$gtt_stage_file" /etc/modprobe.d/amdgpu_llm_optimized.conf 2>/dev/null; then
                ...
                # Keep the staged file: the copy below is the user's only way
                # to apply this tuning, and the previous unconditional cleanup
                # deleted the very file these instructions point at.
                ai_warn "Could not install GTT memory config (needs sudo). Copy manually:"
                ai "  sudo cp $gtt_stage_file /etc/modprobe.d/amdgpu_llm_optimized.conf"
                ai "  sudo update-initramfs -u || sudo dracut --force"
            fi
        fi
    else
