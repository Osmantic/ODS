...
assert_contains "$win_installer" 'Install-ODSLemonadeRuntime -RootPath' "Windows installer should use the shared Lemonade installer"
assert_contains "installers/windows/lib/backend-contract.ps1" 'Start-Process -FilePath msiexec\.exe .* -PassThru' "Windows installer should capture Lemonade MSI exit codes"
assert_contains "installers/windows/lib/backend-contract.ps1" 'Lemonade MSI failed' "Windows installer should report failed Lemonade MSI exit codes honestly"
assert_contains "installers/windows/lib/backend-contract.ps1" 'INSTALLDIR=' "Windows installer should install Lemonade into the normal user's runtime directory"
assert_contains "installers/windows/lib/backend-contract.ps1" '/L\*V' "Windows installer should retain a verbose Lemonade MSI log for support"
assert_not_contains "installers/windows/lib/backend-contract.ps1" 'ALLUSERS=1' "Shared Lemonade installer must not require an elevated all-users install"
