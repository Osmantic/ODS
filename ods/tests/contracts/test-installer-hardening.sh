...
assert_contains "extensions/services/open-webui/compose.yaml" 'RAG_EMBEDDING_ENGINE: "\$\{RAG_EMBEDDING_ENGINE:-openai\}"' "Open WebUI should use API-backed embeddings instead of downloading sentence-transformers at startup"
assert_contains "extensions/services/open-webui/compose.yaml" 'RAG_OPENAI_API_BASE_URL: "\$\{RAG_OPENAI_API_BASE_URL:-http://embeddings:80/v1\}"' "Open WebUI RAG embeddings should route to the bundled TEI service"
assert_contains "extensions/services/open-webui/compose.yaml" 'RAG_EMBEDDING_MODEL_AUTO_UPDATE: "\$\{RAG_EMBEDDING_MODEL_AUTO_UPDATE:-false\}"' "Open WebUI should not auto-update local embedding models during startup"
...
assert_contains "extensions/services/remote-provider-egress/compose.yaml" 'REMOTE_PROVIDER_DATA_GID' "remote-provider services must receive the installation data group"
