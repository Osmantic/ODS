"""Catalog discovery, compose contract, and credential provisioning for Typesense.

Run live docker test: ODS_TEST_TYPESENSE_DOCKER=1 pytest -q tests/test_typesense_extension.py
"""
import json
import os
...
SERVICE = ROOT / 'extensions/library/services/typesense'


def test_typesense_catalog_and_manifest_contract(tmp_path):
    output = tmp_path / 'catalog.json'
    subprocess.run([sys.executable, str(ROOT / 'scripts/generate-extensions-catalog.py'), '--output', str(output)], check=True)
    generated = json.loads(output.read_text())
    entry = next(item for item in generated['extensions'] if item['id'] == 'typesense')
    checked_in = json.loads((ROOT / 'config/extensions-catalog.json').read_text())
    assert entry == next(item for item in checked_in['extensions'] if item['id'] == 'typesense')
    assert entry['port'] == 8108
    assert entry['external_port_default'] == 8108

    # Required secrets check
    required = {item['key'] for item in entry['env_vars'] if item.get('required')}
    assert required == {'TYPESENSE_API_KEY'}

    # Compose contract check
    compose = yaml.safe_load((SERVICE / 'compose.yaml').read_text())
    assert 'typesense' in compose['services']
    service_def = compose['services']['typesense']
    ...
    assert any('TYPESENSE_PORT:-8108}:8108' in p for p in ports)


def test_typesense_setup_generates_credentials_and_directories(tmp_path):
    ...
    assert 'TYPESENSE_PORT=8108' in content
    assert 'TYPESENSE_API_KEY=' in content
    assert (tmp_path / 'data/typesense').is_dir()

    # Verify API key security length
...
@pytest.mark.skipif(os.environ.get('ODS_TEST_TYPESENSE_DOCKER') != '1', reason='Opt-in Docker lifecycle test')
def test_typesense_docker_lifecycle_guard(tmp_path):
    name = f'ods-typesense-test-{uuid.uuid4().hex[:10]}'
    overlay = tmp_path / 'isolation.yaml'
    overlay.write_text(f'''services:
  typesense:
    container_name: {name}
    ports: !override ["127.0.0.1:0:8108"]
networks:
  ods-network:
    name: {name}
''')
    command = ['docker', 'compose', '--project-name', name, '--project-directory', str(tmp_path), '-f', str(SERVICE / 'compose.yaml'), '-f', str(overlay)]
    environment = {key: value for key, value in os.environ.items() if not key.startswith('TYPESENSE_')}

    # Must refuse missing TYPESENSE_API_KEY
    absent = subprocess.run([*command, 'config'], env=environment, capture_output=True, text=True, timeout=30)
    assert absent.returncode != 0
    assert 'TYPESENSE_API_KEY' in absent.stderr
