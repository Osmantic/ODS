...
# llama.cpp RAM prompt cache: a quarter of (memory - 6 GiB), at most a quarter
# of the container limit, at least 512 MiB; nothing when 8192 MiB fits.
assert_eq "$(ods_default_llama_cache_ram_mib 15 12G)" "2304" "16 GB WSL VM (15 GiB) with the 12G 8 GB-GPU profile"
assert_eq "$(ods_default_llama_cache_ram_mib 15 64G)" "2304" "16 GB class with the NVIDIA compose limit"
assert_eq "$(ods_default_llama_cache_ram_mib 16 12G)" "2560" "16 GiB"
assert_eq "$(ods_default_llama_cache_ram_mib 31 27G)" "6400" "32 GB host (31 GiB)"
assert_eq "$(ods_default_llama_cache_ram_mib 62 12G)" "3072" "64 GB host whose profile limits the container to 12G"
assert_eq "$(ods_default_llama_cache_ram_mib 38 34G)" "" "38 GiB keeps llama.cpp's 8192 MiB default"
...
assert_eq "$(ods_default_llama_cache_ram_mib 8 5G)" "512" "8 GiB host reaches the floor"
...
assert_eq "$(phase06_cache nvidia 15 15 12G '' '')" "2304" "phase 06: 16 GB WSL VM with the 8 GB-GPU profile"
assert_eq "$(phase06_cache nvidia 31 15 12G '' '')" "2304" "phase 06: Docker's smaller VM reading wins"
assert_eq "$(phase06_cache nvidia 15 '' 12G '' '')" "2304" "phase 06: host RAM when Docker does not answer"
...
assert_eq "$(phase06_cache intel 15 15 '' '' '')" "2304" "phase 06: Intel Docker llama-server"
