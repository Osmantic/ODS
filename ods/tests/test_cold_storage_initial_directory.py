...
def test_archive_creates_cold_directory_and_moves_model(tmp_path):
    ...
    # Age file access time to 10 days ago (MAX_IDLE_DAYS is 7)
    aged_atime = time.time() - (10 * 86400)
    os.utime(weights, (aged_atime, aged_atime))

    # COLD_DIR deliberately does not exist yet
    ...
    # Verification: cold storage must exist and contain the moved model
