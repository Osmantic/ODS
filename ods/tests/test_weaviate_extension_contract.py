...
SERVICE_DIR = ROOT / "extensions/library/services/weaviate"

def test_weaviate_manifest_structure():
    manifest_path = SERVICE_DIR / "manifest.yaml"
    assert manifest_path.exists(), f"Manifest missing for weaviate"
...
def test_weaviate_compose_validity():
    compose_path = SERVICE_DIR / "compose.yaml"
    if not compose_path.exists():
        compose_path = SERVICE_DIR / "compose.yaml.disabled"
    assert compose_path.exists(), f"Compose missing for weaviate"
