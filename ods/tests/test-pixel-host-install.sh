...
if "qwen" in model["id"].lower() and model["reasoning"] is True:
    ...
    assert agent["params"]["chat_template_kwargs"] == {"enable_thinking": True, **template_switches}
else:
    assert "thinkingDefault" not in agent
    assert model["reasoning"] is False
    assert "compat" not in model
    if "qwen" in model["id"].lower():
        assert agent["params"]["chat_template_kwargs"] == {"enable_thinking": False, **template_switches}
if lean_prompt:
    params = agent["params"]
...
# The local route also keeps earlier Qwen think blocks (preserve_thinking).
check python3 -c 'import json,sys; a=json.load(open(sys.argv[1]))["agents"]["list"][0]; assert a["params"]["chat_template_kwargs"] == {"enable_thinking":False,"preserve_thinking":True}' "$runtime_config"
...
check python3 -c 'import json,sys; v=json.load(open(sys.argv[1])); a=v["agents"]["list"][0]; m=v["models"]["providers"]["ods-local"]["models"][0]; assert m["reasoning"] is True and m["compat"] == {"thinkingFormat":"qwen-chat-template"} and a["thinkingDefault"] == "low" and a["params"]["chat_template_kwargs"] == {"enable_thinking":True,"preserve_thinking":True}' "$reconcile_candidate"
