...
reviewed_sets = list((pathlib.Path(sys.argv[1]).resolve().parents[2] / "extensions/services/pixel-agent/host").glob("openclaw-*.json"))
assert len(set(managed)) == len(managed) == len(reviewed_sets) >= 10
