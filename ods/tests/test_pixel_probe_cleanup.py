...
class CleanupTests(unittest.TestCase):
    def setUp(self):
        ...
        self.scope=str(uuid.uuid4());self.evidence={'settlement':'a'*64,'routerDisarm':'b'*64}
        self.name='authorize-'+'c'*64+'.json';self.file=self.root/self.name
        self.file.write_text(json.dumps({'scopeId':self.scope,'signingKey':'fixture-only'}));self.file.chmod(0o600)
    ...
    def test_actual_owned_cleanup_and_replay_record_absence_without_inventing_removal(self):
        value=self.manifest();self.assertTrue(self.clean(value)['allManifestNamesAbsent'])
        again=self.clean(value);self.assertEqual(again['files'][0]['state'],'absent-unattributed')
    def test_crash_two_links_and_partial_cleanup_reconcile_exact_remaining_names(self):
        claim=self.name+'.claimed';os.link(self.file,self.root/claim)
        ...
        result=self.clean(value);self.assertTrue(result['allManifestNamesAbsent']);self.assertEqual(result['files'][0]['state'],'absent-unattributed')
    def test_changed_replacement_foreign_scope_symlink_and_unapproved_manifest_refused(self):
        value=self.manifest();self.file.write_text(json.dumps({'scopeId':self.scope,'changed':True}))
    ...
    def test_nonmanifest_files_survive_and_evidence_change_refused(self):
        other=self.root/'original-failure-receipt.json';other.write_text('{}')
        value=self.manifest()
        ...
        self.assertTrue(self.clean(value)['allManifestNamesAbsent']);self.assertTrue(other.exists())
