...
def validate_request(payload):
    if (not isinstance(payload,dict) or set(payload)-FIELDS or payload.get('model') != MODEL
            ...
            or ('stop' in payload and not (isinstance(payload['stop'], str) or (isinstance(payload['stop'], list) and len(payload['stop']) <= 4 and all(isinstance(s, str) for s in payload['stop']))))):
