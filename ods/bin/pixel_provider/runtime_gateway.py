...
def validate_request(payload):
    if (not isinstance(payload,dict) or set(payload)-FIELDS or payload.get('model') != MODEL
            ...
            or ('presence_penalty' in payload and (type(payload['presence_penalty']) not in (int, float) or not -2.0 <= float(payload['presence_penalty']) <= 2.0))
            or ('frequency_penalty' in payload and (type(payload['frequency_penalty']) not in (int, float) or not -2.0 <= float(payload['frequency_penalty']) <= 2.0))):
