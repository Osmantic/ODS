...
def validate_request(payload):
    if (not isinstance(payload,dict) or set(payload)-FIELDS or payload.get('model') != MODEL
            ...
            or ('logprobs' in payload and type(payload['logprobs']) is not bool)
            or ('top_logprobs' in payload and (type(payload['top_logprobs']) is not int or not 0 <= payload['top_logprobs'] <= 20 or not payload.get('logprobs', False)))):
