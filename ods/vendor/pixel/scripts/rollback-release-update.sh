...
usage() {
  pixel_die "Usage: ./pixel update-rollback --preview --candidate-id ID --allowed-signers ABSOLUTE_PATH --identity IDENTITY --activation-hash FULL_SHA256 | ./pixel update-rollback --candidate-id ID --allowed-signers ABSOLUTE_PATH --identity IDENTITY --activation-hash FULL_SHA256 --rollback-hash FULL_SHA256 --confirm"
}
...
if [[ ${1:-} == --preview ]]; then
  shift
  exec python3 "$ROOT/scripts/release-update.py" rollback-preview \
    "$@" \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --rollback-hash)
      [[ -z "$rollback_hash" && $# -ge 2 ]] || usage
      rollback_hash=$2; shift 2 ;;
    --confirm)
      [[ $confirmed == 0 ]] || usage
      confirmed=1; shift ;;
    *) usage ;;
...
[[ "$activation_hash" =~ ^[0-9a-f]{64}$ && "$rollback_hash" =~ ^[0-9a-f]{64}$ && $confirmed == 1 ]] || usage
...
[[ -f "$rollback_marker" && ! -L "$rollback_marker" ]] || pixel_die "The activated update rollback marker is unavailable or unsafe"

python3 "$ROOT/scripts/release-update.py" rollback-claim \
  --staging-root "$staging_root" --candidate-id "$candidate_id" \
  --allowed-signers "$allowed_signers" --identity "$identity" \
  --activation-hash "$activation_hash" --rollback-hash "$rollback_hash" \
...
record_failure() {
  ...
  if [[ $status != 0 && $rollback_started == 1 ]]; then
    python3 "$ROOT/scripts/release-update.py" rollback-result \
      --staging-root "$staging_root" --candidate-id "$candidate_id" \
      --allowed-signers "$allowed_signers" --identity "$identity" \
      --activation-hash "$activation_hash" --rollback-hash "$rollback_hash" \
      --active-version-file "$active_version_file" --rollback-marker "$rollback_marker" \
      --outcome failed --phase rollback --confirm >/dev/null \
      || pixel_warn "Update rollback failed and its content-free result could not be recorded; preserve update staging for recovery"
  fi
  exit "$status"
}
...
result=$(python3 "$ROOT/scripts/release-update.py" rollback-result \
  --staging-root "$staging_root" --candidate-id "$candidate_id" \
  --allowed-signers "$allowed_signers" --identity "$identity" \
  --activation-hash "$activation_hash" --rollback-hash "$rollback_hash" \
  --active-version-file "$active_version_file" --rollback-marker "$rollback_marker" \
  --outcome rolled-back --phase record --confirm)
trap - EXIT
printf '%s\n' "$result"
