...
pixel_broker_byte_files() {
  case "$1" in
    # The installed source-broker runtime is a single self-contained broker.py byte: the
    # action-journal and vendor-secret helpers are bundled inside it, so the new install
    # contract splits to install only broker.py.
    source) printf '%s\n' 'broker.py|0755|source-broker/broker.py' ;;
...
# Legacy backup-manifest contract, preserved exactly so an older trusted controller whose
# source broker-byte manifest knows only broker.py and the (absent) action_journal package
# can restore a new-format activation backup without an "unexpected records" failure.
pixel_broker_backup_files() {
  case "$1" in
    source) printf '%s\n%s\n' 'broker.py|0755|source-broker/broker.py' 'action_journal/__init__.py|0644|source-broker/action_journal/__init__.py' ;;
    *) pixel_broker_byte_files "$1" ;;
  esac
}

pixel_broker_byte_dirs() {
  case "$1" in
    source) return 0 ;;
    ops|frontier) return 0 ;;
    *) return 1 ;;
  esac
}
...
pixel_broker_bytes_via_release_operator() {
  pixel_release_operator_enabled && [[ ${PIXEL_BROKER_BYTES_DIRECT_SUDO:-0} != 1 ]]
}

pixel_broker_bytes_require_trust() {
  local limbs limb install_dir
  pixel_broker_validate_configuration
  if pixel_broker_bytes_via_release_operator; then
    # Capability probe only: the root-owned operator config/runtime are the authority.
    # Do not discover privileged paths or units from caller environment in this mode.
    pixel_release_operator_client --validate-config >/dev/null
    return 0
  fi
  ...
  for limb in $limbs; do
    ...
    pixel_broker_require_installed_dir "$install_dir" 0755
...
pixel_broker_create_dir() {
  local target=$1 mode=$2
  if pixel_broker_bytes_test_mode; then
    install -d -m "$mode" "$target"
  else
    sudo install -d -o root -g root -m "$mode" "$target"
  fi
}
...
pixel_broker_remove_dir() {
  local target=$1
  if pixel_broker_bytes_test_mode; then rmdir -- "$target"; else sudo rmdir -- "$target"; fi
}
...
pixel_broker_require_installed_dir() {
  local path=$1 mode=$2 info
  [[ -d "$path" && ! -L "$path" ]] || pixel_die "Installed broker directory is missing or linked: $path"
  info=$(stat -c '%a:%U:%G' -- "$path")
  if pixel_broker_bytes_test_mode; then
    [[ "$info" == "${mode#0}:"* ]] || pixel_die "Installed broker directory has unsafe metadata: $path"
  else
    [[ "$info" == "${mode#0}:root:root" ]] || pixel_die "Installed broker directory must be root:root:${mode#0}: $path"
  fi
}

pixel_broker_prepare_dirs() {
  local limb=$1 install_dir=$2 relative mode target parent
  # Revalidate the privileged parent immediately before child creation. This is
  # stronger than relying only on the transaction's earlier trust preflight and
  # keeps the directory path aligned with the existing file-destination guard.
  pixel_broker_require_installed_dir "$install_dir" 0755
  while IFS='|' read -r relative mode; do
    [[ -n "$relative" && "$relative" != */* ]] || pixel_die "Broker-byte directory contract must name one fixed child: $limb/$relative"
    target="$install_dir/$relative"
    parent=$(dirname "$target")
    [[ "$parent" == "$install_dir" ]] || pixel_die "Broker-byte directory escapes the install root: $limb/$relative"
    if [[ -e "$target" || -L "$target" ]]; then
      pixel_broker_require_installed_dir "$target" "$mode"
    else
      pixel_broker_create_dir "$target" "$mode"
      pixel_broker_require_installed_dir "$target" "$mode"
    fi
  done < <(pixel_broker_byte_dirs "$limb")
}
...
pixel_broker_bytes_backup_all() {
  ...
  if pixel_broker_bytes_via_release_operator; then
    pixel_release_operator_client broker-bytes backup
    return 0
  fi
  pixel_broker_bytes_require_trust
  ...
  for limb in $(pixel_broker_limbs); do
    ...
    : > "$limb_root/directories.tsv"
    chmod 600 "$limb_root/manifest.tsv"
    chmod 600 "$limb_root/directories.tsv"
    while IFS='|' read -r relative mode; do
      [[ -n "$relative" && "$relative" != */* ]] || pixel_die "Broker-byte directory contract must name one fixed child: $limb/$relative"
      if [[ -e "$install_dir/$relative" || -L "$install_dir/$relative" ]]; then
        pixel_broker_require_installed_dir "$install_dir/$relative" "$mode"
        state=present
      else
        state=absent
      fi
      printf '%s|%s|%s\n' "$relative" "$state" "$mode" >> "$limb_root/directories.tsv"
    done < <(pixel_broker_byte_dirs "$limb")
    ...
    done < <(pixel_broker_backup_files "$limb")
  done
}
...
pixel_broker_bytes_install_all() {
  ...
  if pixel_broker_bytes_via_release_operator; then
    pixel_release_operator_client broker-bytes install
    return 0
  fi
  pixel_broker_bytes_require_trust
  ...
  for limb in $(pixel_broker_limbs); do
    install_dir=$(pixel_broker_install_dir "$limb")
    pixel_broker_prepare_dirs "$limb" "$install_dir"
    while IFS='|' read -r relative mode release_relative; do
...
pixel_broker_bytes_restore_all() {
  ...
  if pixel_broker_bytes_via_release_operator; then
    pixel_release_operator_client broker-bytes restore
    return 0
  fi
  ...
  for limb in source ops frontier; do
    ...
    exec 3<&-
    [[ -f "$limb_root/directories.tsv" && ! -L "$limb_root/directories.tsv" ]] || pixel_die "Broker-byte directory backup is missing: $limb"
    exec 3< "$limb_root/directories.tsv"
    while IFS='|' read -r relative mode; do
      IFS='|' read -r record_relative state record_mode <&3 || pixel_die "Broker-byte directory backup is truncated: $limb"
      [[ "$record_relative" == "$relative" && "$record_mode" == "$mode" ]] || pixel_die "Broker-byte directory backup does not match the fixed directory contract: $limb"
      case "$state" in
        present)
          pixel_broker_require_installed_dir "$install_dir/$relative" "$mode"
          ;;
        absent)
          if [[ -e "$install_dir/$relative" || -L "$install_dir/$relative" ]]; then
            pixel_broker_require_installed_dir "$install_dir/$relative" "$mode"
            pixel_broker_remove_dir "$install_dir/$relative"
          fi
          ;;
        *) pixel_die "Invalid broker-byte directory backup state: $limb/$relative" ;;
      esac
    done < <(pixel_broker_byte_dirs "$limb")
    if IFS= read -r observed <&3; then pixel_die "Broker-byte directory backup has unexpected records: $limb"; fi
    exec 3<&-
...
pixel_broker_bytes_verify() {
  ...
  if pixel_broker_bytes_via_release_operator; then
    pixel_release_operator_client broker-bytes verify
    return 0
  fi
  pixel_broker_validate_configuration
  for limb in $(pixel_broker_limbs); do
