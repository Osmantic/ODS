...
usage() {
  pixel_die "Usage: ./pixel update-cleanup --preview --candidate-id ID --allowed-signers ABSOLUTE_PATH --identity IDENTITY --activation-hash FULL_SHA256 | ./pixel update-cleanup --candidate-id ID --allowed-signers ABSOLUTE_PATH --identity IDENTITY --activation-hash FULL_SHA256 --cleanup-hash FULL_SHA256 --confirm"
}
...
if [[ ${1:-} == --preview ]]; then
  shift
  exec python3 "$ROOT/scripts/release-update.py" cleanup-preview \
    "$@" \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --cleanup-hash)
      [[ -z "$cleanup_hash" && $# -ge 2 ]] || usage
      cleanup_hash=$2; shift 2 ;;
    --confirm)
      [[ $confirmed == 0 ]] || usage
      confirmed=1; shift ;;
    *) usage ;;
...
[[ "$activation_hash" =~ ^[0-9a-f]{64}$ && "$cleanup_hash" =~ ^[0-9a-f]{64}$ && $confirmed == 1 ]] || usage

exec python3 "$ROOT/scripts/release-update.py" cleanup \
  --staging-root "$staging_root" --candidate-id "$candidate_id" \
  --allowed-signers "$allowed_signers" --identity "$identity" \
  --activation-hash "$activation_hash" --cleanup-hash "$cleanup_hash" \
