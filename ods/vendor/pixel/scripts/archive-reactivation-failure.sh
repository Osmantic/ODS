...
usage() {
  pixel_die "Usage: ./pixel update-reactivation-archive --preview --candidate-id ID --controller-envelope ABSOLUTE_PATH --allowed-signers ABSOLUTE_PATH --identity IDENTITY --activation-hash FULL_SHA256 | ./pixel update-reactivation-archive --candidate-id ID --controller-envelope ABSOLUTE_PATH --allowed-signers ABSOLUTE_PATH --identity IDENTITY --activation-hash FULL_SHA256 --archive-hash FULL_SHA256 --confirm"
}
...
archive_root="$PIXEL_INSTALL_DIR/update-reactivation-archive"
...
if [[ ${1:-} == --preview ]]; then
  shift
  exec python3 "$ROOT/scripts/release-update.py" reactivation-archive-preview \
    "$@" \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --identity)
      ...
      identity=$2; shift 2 ;;
    --controller-envelope)
      [[ -z "$controller_envelope" && $# -ge 2 ]] || usage
      controller_envelope=$2; shift 2 ;;
    --activation-hash)
...
[[ "$allowed_signers" == /* && "$controller_envelope" == /* && -n "$identity" ]] || usage
[[ "$activation_hash" =~ ^[0-9a-f]{64}$ && "$archive_hash" =~ ^[0-9a-f]{64}$ && $confirmed == 1 ]] || usage
pixel_require_interactive_terminal

exec python3 "$ROOT/scripts/release-update.py" reactivation-archive \
  --staging-root "$staging_root" --archive-root "$archive_root" \
  --controller-root "$ROOT" --controller-envelope "$controller_envelope" \
