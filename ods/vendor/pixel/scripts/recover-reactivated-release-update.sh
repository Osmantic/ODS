...
usage() {
  pixel_die "Usage: ./pixel update-reactivation-recover --preview --candidate-id ID --allowed-signers ABSOLUTE_PATH --identity IDENTITY --activation-hash FULL_SHA256 --reactivation-hash FULL_SHA256 | ./pixel update-reactivation-recover --candidate-id ID --allowed-signers ABSOLUTE_PATH --identity IDENTITY --activation-hash FULL_SHA256 --reactivation-hash FULL_SHA256 --recovery-hash FULL_SHA256 --confirm"
}
...
if [[ ${1:-} == --preview ]]; then
  shift
  exec python3 "$ROOT/scripts/release-update.py" reactivation-recovery-preview \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --recovery-hash) [[ -z "$recovery_hash" && $# -ge 2 ]] || usage; recovery_hash=$2; shift 2 ;;
...
[[ "$recovery_hash" =~ ^[0-9a-f]{64}$ && $confirmed == 1 ]] || usage

exec python3 "$ROOT/scripts/release-update.py" reactivation-recovery-finalize \
  ...
  --recovery-hash "$recovery_hash" --active-version-file "$active_version_file" \
