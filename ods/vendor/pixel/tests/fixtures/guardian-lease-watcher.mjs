...
// ("node <helper> watch --config <configPath>") and publishes its own guardian
// lease from its own live /proc identity, then stays alive. It is used only to
// prove a real separate-process watcher lease is accepted by the proof path.
import { readFile } from "node:fs/promises";
import { refreshGuardianLease } from "../../deploy/work-controller/maintenance-recovery-guardian-lease.mjs";
...
await refreshGuardianLease(custodyLockPath, uid, configSha256, { unit, custodyIdentitySha256, guardianModuleSha256, expectedUnitSha256, expectedNodePath, ready: false });
await refreshGuardianLease(custodyLockPath, uid, configSha256, { unit, custodyIdentitySha256, guardianModuleSha256, expectedUnitSha256, expectedNodePath, ready: true });
