...
This is a historical status review of the March 2026 static security audit,
last evaluated in May 2026. Its remediation labels apply to that reviewed tree,
not automatically to the September main promotion or a running installation.
Absence of an open item here does not establish that current source is secure.
Use the current security policy and commit-bound release evidence for current
decisions; do not treat this historical table as a current vulnerability scan.
...
| H3 | OpenClaw gateway combined disabled device auth with LAN binding | Remediated in tree | OpenClaw is deprecated and optional in [`ods/extensions/services/openclaw/manifest.yaml`](ods/extensions/services/openclaw/manifest.yaml); compose requires `OPENCLAW_TOKEN`; static configs set `dangerouslyDisableDeviceAuth` false; [`ods/tests/contracts/test-network-exposure-contracts.py`](ods/tests/contracts/test-network-exposure-contracts.py) keeps OpenClaw opt-in and token-gated. | Keep standalone OpenClaw legacy-only. For current Portal/Pixel eligibility and Hermes fallback, use [the agent guide](ods/docs/PIXEL.md). |
...
| M4 | Voice agent defaulted to unencrypted `ws://` | Mitigated / accepted local risk | The May review described HTTPS-aware WebSocket selection in the now-removed `useVoiceAgent.js` hook. Current [ODS Talk](ods/extensions/services/dashboard/src/pages/ODSTalk.jsx) needs its own transport review; the removed hook is not current evidence. [`ods/SECURITY.md`](ods/SECURITY.md) recommends TLS or VPN for network exposure. | Plain HTTP on localhost/LAN remains cleartext by design; use TLS or Tailscale/WireGuard for sensitive shared deployments. |
