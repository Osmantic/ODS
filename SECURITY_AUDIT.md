...
| M4 | Voice agent defaulted to unencrypted `ws://` | Resolved — surface removed | The LiveKit voice agent UI (`useVoiceAgent.js`, `Voice.jsx`) was removed in [#1380](https://github.com/Osmantic/ODS/pull/1380); the `ws://` client no longer ships. [`ods/SECURITY.md`](ods/SECURITY.md) recommends TLS or VPN for any future voice exposure. | Keep voice/realtime features behind TLS or Tailscale/WireGuard if reintroduced. |
